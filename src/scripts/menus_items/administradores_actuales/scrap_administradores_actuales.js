...
/**
 * Scrap administradores actuales
 *
 * @param {} page
 * @param {} company
 */
export default async (page, company) => {
    // query the page pf the administradores actuales
    console.log('sending query to change to administradores actuales');
    let number_of_rows = await send_request(
        query_administradores_actuales,
        // the callback, this is goin to run in the browser,
        (response, status, i, C) => 
            window.number_of_rows_administradores_actuales(response),
        page, // puppetter page
