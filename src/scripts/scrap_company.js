 ...
 *
 */
const script = async (company, proxy=false, log_color) => { 

    // get company from company id
    //let company = get_company_by_id(company_id);
    // make logger
    let prefix = `[${company.ruc}] `;
    ...
    log(`scrapping ${company.ruc} through ${proxy}`)
    // create new browser
...
//await script({"id":"316620","name":"'' ALERTA10 S.A.'' SOCIEDAD ANÓNIMA","ruc":"1793043429001"})
